...
VARIABLES memory, cache, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer, bus_mode, upgrading, upgraded, state, owning, owned, transfer

vars == <<memory, cache, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer, bus_mode, upgrading, upgraded, state, owning, owned, transfer>>
...
issue_proc_read_invalid(c, a) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owning, owned, transfer>>
...
do_bus_read_invalid(c, a) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, owning, owned, transfer>>
...
do_bus_read_valid(c, a, v) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, owning, owned>>
    /\ LET state_val == IF (state[c][a] = "read") THEN "bad" ELSE state[c][a] IN
       state' = [state EXCEPT![c][a] = state_val]
    /\ transfer' = TRUE
...
complete_proc_read_invalid_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owning, owned>>
...
issue_proc_write_invalid(c, a, v) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owned, transfer>>
...
do_bus_read_for_ownership_invalid(c, a) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, state, owning, transfer>>
    /\ owned' = TRUE
...
do_bus_read_for_ownership_valid(c, a, v) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, state, owning>>
    /\ owned' = TRUE
    /\ transfer' = TRUE
...
issue_proc_write_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<upgraded, owning, owned, transfer>>
    /\ bus_mode' = IF bus_mode = "not_used" THEN "write" ELSE "bad"
...
do_bus_upgrade(c, a) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, state, owning, owned, transfer>>
...
complete_proc_write_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<owning, owned, transfer>>
