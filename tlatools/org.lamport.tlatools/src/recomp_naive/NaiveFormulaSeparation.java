...
import recomp.ExtendTraceVisitor;
import recomp.Formula;
import recomp.FormulaSynth;
import recomp.InitTraceVisitor;
import recomp.NegTraceGen;
import recomp.PerfTimer;
...
public class NaiveFormulaSeparation {
	...
	public NaiveFormulaSeparation(final String tlaComp, final String cfgComp, final String tlaRest, final String cfgRest,
	...
	public Set<Formula> minimalSetOfSepInvariants() {
    	...
    	int largestFluentNumSeen = this.useIntermediateProp ? this.intermediateProp.getMaxFluentNum() + 1 : 0;
    	
    	Set<AlloyTrace> negTraces = new HashSet<>();
    	Set<AlloyTrace> posTraces = new HashSet<>();
    	Formula invariant = Formula.TRUE();
    	
    	posTraces.add(defaultInitPosTrace);
		...
		boolean hasNegTrace = true;
		boolean hasPosTrace = true;
		
    	while (hasNegTrace || hasPosTrace) {
    		do {
            	final String tlaCompHV = writeHistVarsSpec(tlaComp, cfgComp, invariant, true);
            	final AlloyTrace negTrace = genNegCexTraceForCandSepInvariant(tlaCompHV, cfgNegTraces, "NT", negTraces.size()+1, "NegTrace", NEG_TRACE_TIMEOUT);
            	hasNegTrace = negTrace.hasError() && negTrace.rawTrace() != null;
            	if (hasNegTrace) {
            		System.out.println();
            		System.out.println("Found a new negative trace:");
            		System.out.println(negTrace);
            		negTraces.add(negTrace.restrictToAlphabet(this.globalActions));
            		System.out.println("Synthesizing an invariant with " + negTraces.size() + " neg traces and " + posTraces.size() + " pos traces");
        			invariant = synthesizeFormulas(negTraces, posTraces, largestFluentNumSeen, envVarTypes);
        			System.out.println("Synthesized the following invariant:");
        			System.out.println(invariant);
            		if (invariant.isUNSAT()) {
            			return Utils.setOf(invariant);
            		}
            	}
    		} while(false);
    		...
    		do {
				...
				final String tlaRestHV = writeHistVarsSpec(tlaRest, cfgRest, invariant, false);
				final AlloyTrace posTrace =
						genCexTraceForCandSepInvariant(tlaRestHV, cfgPosTraces, "PT", posTraces.size()+1, "PosTrace", fiveMinuteTimeout);
            	hasPosTrace = posTrace.hasError() && posTrace.rawTrace() != null;
            	if (hasPosTrace) {
            		System.out.println();
            		System.out.println("Found a new positive trace:");
            		System.out.println(posTrace);
            		posTraces.add(posTrace.restrictToAlphabet(this.globalActions));
            		System.out.println("Synthesizing an invariant with " + negTraces.size() + " neg traces and " + posTraces.size() + " pos traces");
        			invariant = synthesizeFormulas(negTraces, posTraces, largestFluentNumSeen, envVarTypes);
        			System.out.println("Synthesized the following invariant:");
        			System.out.println(invariant);
	...
	private AlloyTrace createDefaultInitPosTrace() {
    	...
    	while (initPosTrace.isEmpty()) {
        	...
        	initPosTrace = createAlloyTrace(initTrace, "PT1", "PosTrace").restrictToAlphabet(this.globalActions);
        	++initTraceLen;
    	}
	...
	private Formula synthesizeFormulas(final Set<AlloyTrace> negTraces,
			final Set<AlloyTrace> posTraces, final int curNumFluents, Set<Map<String,String>> envVarTypes) {
		NaiveFormulaSynth formSynth = new NaiveFormulaSynth(this.seed);
		return formSynth.synthesizeFormulas(envVarTypes, negTraces, posTraces,
