...
pred isInitAct[a : Act, e : Env, f : Fluent] {
    ...
    let s = highestPriorityFlAct[flActCategory] |
        ...
        (s.value = True and a.baseName = s.baseName and (~(f.vars)).(s.prioritizedFlToActParamsMap).(a.params) in e.maps)
        or
        // mutex init fluents
        (s.mutexFl = True and s.value = False and a.baseName = s.baseName and (~(f.vars)).(s.prioritizedFlToActParamsMap).(a.params) not in e.maps)
}
...
pred isTermAct[a : Act, e : Env, f : Fluent] {
    ...
    let s = highestPriorityFlAct[flActCategory] |
        // init fluents, both usual and mutex
        (s.value = False and a.baseName = s.baseName and (~(f.vars)).(s.prioritizedFlToActParamsMap).(a.params) in e.maps)
        or
        // mutex init fluents
        (s.mutexFl = True and s.value = True and a.baseName = s.baseName and (~(f.vars)).(s.prioritizedFlToActParamsMap).(a.params) not in e.maps)
}
...
fact PartialInstance {
	lastIdx = (EmptyTrace->T0) + (PT34->T4) + (PT28->T4) + (PT25->T4) + (PT31->T5) + (PT35->T4) + (PT27->T3) + (PT32->T4) + (PT22->T3) + (PT37->T5) + (PT30->T5) + (PT36->T4) + (PT33->T3) + (PT26->T4) + (PT1->T5) + (PT18->T2) + (NT1->T4)
