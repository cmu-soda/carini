...
pred isInitAct[a : Act, e : Env, f : Fluent] {
    (some s : f.flActions |
        s.flActIdx = S0 and s.value = True and a.baseName = s.baseName and (~(f.vars)).(s.flToActParamsMap).(a.params) in e.maps)
...
pred isTermAct[a : Act, e : Env, f : Fluent] {
    (some s : f.flActions |
        s.flActIdx = S0 and s.value = False and a.baseName = s.baseName and (~(f.vars)).(s.flToActParamsMap).(a.params) in e.maps)
    //let flActCategory = { s : f.flActions | s.baseName = a.baseName } |
    //let si = highestPriority[flActCategory] |
...
fact PartialInstance {
	...
	path = (PT2 -> (T0->SndPreparer1 + T1->SndPreparer2 + T2->SndPreparer3 + T3->RcvCommitr1)) +
		(PT1 -> (T0->SndPreparer1 + T1->SndPreparer2 + T2->RcvAbortr1 + T3->SndPreparer3 + T4->SndPreparer1)) +
		(NT1 -> (T0->SndPreparer1 + T1->SndPreparer2 + T2->RcvAbortr1 + T3->SndPreparer3 + T4->RcvCommitr2))
