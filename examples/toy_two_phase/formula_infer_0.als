...
one sig Commitr1 extends Act {} {
	params = (P0->r1)
}
one sig Commitr2 extends Act {} {
	params = (P0->r2)
}
...
one sig Preparer3 extends Act {} {
	params = (P0->r3)
}
one sig Preparer1 extends Act {} {
	params = (P0->r1)
}
one sig Preparer2 extends Act {} {
	params = (P0->r2)
}
...
fact {
	...
	Commit in FlSymAction.baseName // the final base name in the neg trace must appear in the sep formula
}
...
fact PartialInstance {
	lastIdx = (EmptyTrace->T0) + (PT1->T4) + (PT2->T3) + (NT1->T4)

	path = (PT1 -> (T0->Preparer1 + T1->Preparer2 + T2->Preparer3 + T3->Abortr1 + T4->Abortr1)) +
		(PT2 -> (T0->Preparer2 + T1->Preparer1 + T2->Preparer3 + T3->Commitr1)) +
		(NT1 -> (T0->Preparer1 + T1->Preparer2 + T2->Preparer3 + T3->Abortr1 + T4->Commitr2))
