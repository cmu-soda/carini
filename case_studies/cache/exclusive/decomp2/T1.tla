...
VARIABLES modified, shared, invalid, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer

vars == <<modified, shared, invalid, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer>>
...
issue_proc_read_invalid(c, a) ==
    ...
    /\ UNCHANGED<<modified, shared, invalid, proc_write, bus_read_for_ownership, bus_upgrade, bus_transfer>>

do_bus_read_invalid(c, a) ==
    /\ bus_read[c][a]
    /\ invalid[c][a]
    /\ bus_read' = [bus_read EXCEPT![c][a] = FALSE]
    /\ UNCHANGED<<modified, shared, invalid, proc_read, proc_write, bus_in_use, bus_read_for_ownership, bus_upgrade, bus_transfer>>
...
complete_proc_read_invalid_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<modified, proc_write, bus_read, bus_read_for_ownership, bus_upgrade>>
...
complete_proc_read_invalid_exclusive(c, a, v) ==
    ...
    /\ bus_in_use' = FALSE
    /\ proc_read' = [proc_read EXCEPT![c][a] = FALSE]
    /\ UNCHANGED<<modified, shared, proc_write, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer>>
...
issue_proc_write_invalid(c, a, v) ==
    ...
    /\ UNCHANGED<<modified, shared, invalid, proc_read, bus_read, bus_upgrade, bus_transfer>>
...
do_bus_read_for_ownership_invalid(c, a) ==
    ...
    /\ UNCHANGED<<modified, shared, invalid, proc_read, proc_write, bus_in_use, bus_read, bus_upgrade, bus_transfer>>
...
complete_proc_write_invalid(c, a, v) ==
    ...
    /\ UNCHANGED<<shared, proc_read, bus_read, bus_read_for_ownership, bus_upgrade>>

proc_write_exclusive(c, a, v) ==
    /\ ~bus_in_use
    /\ modified' = [modified EXCEPT![c][a] = TRUE]
    /\ UNCHANGED<<shared, invalid, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer>>
...
issue_proc_write_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<modified, shared, invalid, proc_read, bus_read, bus_read_for_ownership, bus_transfer>>
...
do_bus_upgrade(c, a) ==
    ...
    /\ invalid' = [invalid EXCEPT![c][a] = TRUE]
    /\ shared' = [shared EXCEPT![c][a] = FALSE]
    /\ UNCHANGED<<modified, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_transfer>>
...
evict_exclusive(c, a) ==
    ...
    /\ shared' = [shared EXCEPT![c][a] = FALSE]
    /\ invalid' = [invalid EXCEPT![c][a] = TRUE]
    /\ UNCHANGED<<modified, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer>>
