...
abstract sig Trace {
    ...
    initiates : Idx -> FlSymAction, // the actions that initiate fluent values
    terminates : Idx -> FlSymAction, // the actions that terminate fluent values
	satisfies : Env -> Idx -> Formula // formulas that satisfy this trace
} {
    // fluent actions that initiate
    all i : Idx, s : FlSymAction | i->s in initiates iff
        let a = i.path |
        let si = s.flActIdx |
            isInitAct[a,s]
            or 
            (not isTermAct[a,s] and some si.prev and (si.prev)->s in initiates)

    // fluent actions that terminate
    all i : Idx, s : FlSymAction | i->s in terminates iff
        let a = i.path |
        let si = s.flActIdx |
            isTermAct[a,s]
            or 
            (not isInitAct[a,s] and some si.prev and (si.prev)->s in terminates)
	...
	all e : Env, i : Idx, f : Fluent | e->i->f in satisfies iff
        ...
        let s = highestPriorityFlSymAction[f,a] |
            ((~(f.vars)).(s.flToActParamsMap).(a.params) in e.maps) and
            ((i->s in initiates) or
             (i->s not in terminates and no i.prev and f.initially = True) or
             (i->s not in terminates and some i.prev and e->(i.prev)->f in satisfies))
...
pred isInitAct[a : Act, s : FlSymAction] {
    s.value = True and a.baseName = s.baseName
}

pred isTermAct[a : Act, s : FlSymAction] {
    s.value = False and a.baseName = s.baseName
}
