...
VARIABLES memory, cache, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer, bus_mode, upgrading, upgraded, state, owning, owned

vars == <<memory, cache, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer, bus_mode, upgrading, upgraded, state, owning, owned>>
...
issue_proc_write_invalid(c, a, v) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owned>>
...
issue_proc_write_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<upgraded, owning, owned>>
    /\ bus_mode' = IF bus_mode = "not_used" THEN "write" ELSE "bad"
    /\ upgrading' = TRUE
    /\ LET state_val == IF (\A C \in Core, A \in Address : state[C][A] = "NA") THEN "write" ELSE "bad" IN
       state' = [state EXCEPT![c][a] = state_val]
...
do_bus_upgrade(c, a) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, state, owning, owned>>
    /\ upgraded' = TRUE
