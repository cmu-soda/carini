...
VARIABLES memory, cache, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer, bus_mode, upgrading, upgraded, state, owning, owned, transfer, read_state, write_state

vars == <<memory, cache, proc_read, proc_write, bus_in_use, bus_read, bus_read_for_ownership, bus_upgrade, bus_transfer, bus_mode, upgrading, upgraded, state, owning, owned, transfer, read_state, write_state>>
...
issue_proc_read_invalid(c, a) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owning, owned, transfer, write_state>>
...
do_bus_read_invalid(c, a) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, owning, owned, transfer, write_state>>
...
do_bus_read_valid(c, a, v) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, owning, owned, write_state>>
    /\ LET state_val == IF (state[c][a] = "read") THEN "bad" ELSE state[c][a] IN
       state' = [state EXCEPT![c][a] = state_val]
    /\ transfer' = TRUE
    /\ LET read_state_val == IF (read_state[c][a] = "read") THEN "NA" ELSE "bad" IN
...
complete_proc_read_invalid_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owning, owned, write_state>>
...
issue_proc_write_invalid(c, a, v) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owned, transfer, read_state>>
...
do_bus_read_for_ownership_invalid(c, a) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, state, owning, transfer, read_state>>
    /\ owned' = TRUE
...
do_bus_read_for_ownership_valid(c, a, v) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, upgraded, state, owning, read_state>>
    /\ owned' = TRUE
    /\ transfer' = TRUE
    /\ LET write_state_val == IF (write_state[c][a] = "write") THEN "NA" ELSE "bad" IN
       write_state' = [write_state EXCEPT![c][a] = write_state_val]
...
proc_write_exclusive(c, a, v) ==
    ...
    /\ UNCHANGED<<upgrading, upgraded, owning, owned, transfer, read_state, write_state>>
...
issue_proc_write_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<upgraded, owning, owned, transfer, read_state, write_state>>
    /\ bus_mode' = IF bus_mode = "not_used" THEN "write" ELSE "bad"
...
do_bus_upgrade(c, a) ==
    ...
    /\ UNCHANGED<<bus_mode, upgrading, state, owning, owned, transfer, read_state, write_state>>
...
complete_proc_write_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<owning, owned, transfer, read_state, write_state>>
