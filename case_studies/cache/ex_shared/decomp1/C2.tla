...
VARIABLES bus_read, bus_upgrade

\* mutable relation bus_transfer(value)
VARIABLE bus_transfer

vars == <<memory, cache, modified, invalid, proc_read, proc_write, bus_in_use, bus_read, bus_upgrade, bus_transfer>>
...
issue_proc_read_invalid(c, a) ==
    ...
    /\ UNCHANGED<<memory, cache, modified, invalid, proc_write, bus_upgrade, bus_transfer>>

do_bus_read_invalid(c, a) ==
    /\ bus_read[c][a]
    /\ invalid[c][a]
    /\ bus_read' = [bus_read EXCEPT![c][a] = FALSE]
    /\ UNCHANGED<<memory, cache, modified, invalid, proc_read, proc_write, bus_in_use, bus_upgrade, bus_transfer>>
...
do_bus_read_valid(c, a, v) ==
    ...
    /\ UNCHANGED<<cache, invalid, proc_read, proc_write, bus_in_use, bus_upgrade>>
...
complete_proc_read_invalid_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<memory, modified, proc_write, bus_read, bus_upgrade>>
...
complete_proc_read_invalid_exclusive(c, a, v) ==
    ...
    /\ bus_in_use' = FALSE
    /\ proc_read' = [proc_read EXCEPT![c][a] = FALSE]
    /\ UNCHANGED<<memory, modified, proc_write, bus_read, bus_upgrade, bus_transfer>>
...
complete_proc_write_invalid(c, a, v) ==
    ...
    /\ UNCHANGED<<memory, proc_read, bus_read, bus_upgrade>>

proc_write_exclusive(c, a, v) ==
    /\ ~bus_in_use
    /\ modified' = [modified EXCEPT![c][a] = TRUE]
    /\ cache' = [cache EXCEPT![c][a] = v]
    /\ UNCHANGED<<memory, invalid, proc_read, proc_write, bus_in_use, bus_read, bus_upgrade, bus_transfer>>
...
issue_proc_write_shared(c, a, v) ==
    ...
    /\ UNCHANGED<<memory, cache, modified, invalid, proc_read, bus_read, bus_transfer>>

do_bus_upgrade(c, a) ==
    /\ bus_upgrade[c][a]
    /\ bus_upgrade' = [bus_upgrade EXCEPT![c][a] = FALSE]
    /\ invalid' = [invalid EXCEPT![c][a] = TRUE]
    /\ UNCHANGED<<memory, cache, modified, proc_read, proc_write, bus_in_use, bus_read, bus_transfer>>
...
complete_proc_write_shared(c, a, v) ==
    ...
    /\ proc_write' = [proc_write EXCEPT![c][a][v] = FALSE]
    /\ bus_in_use' = FALSE
    /\ UNCHANGED<<memory, invalid, proc_read, bus_read, bus_upgrade, bus_transfer>>
...
evict_modified(c, a) ==
    ...
    /\ modified' = [modified EXCEPT![c][a] = FALSE]
    /\ invalid' = [invalid EXCEPT![c][a] = TRUE]
    /\ UNCHANGED<<cache, proc_read, proc_write, bus_in_use, bus_read, bus_upgrade, bus_transfer>>

evict_exclusive_or_shared(c, a) ==
    /\ ~bus_in_use
    /\ invalid' = [invalid EXCEPT![c][a] = TRUE]
    /\ UNCHANGED<<memory, cache, modified, proc_read, proc_write, bus_in_use, bus_read, bus_upgrade, bus_transfer>>
