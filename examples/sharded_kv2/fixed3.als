...
abstract sig Act {
	...
	initiates : Fluent->FlActionIdx,
	terminates : Fluent->FlActionIdx
}

fact {
    // initiation for a concrete action (Act)
    all a : Act, f : Fluent, si : FlActionIdx | f->si in a.initiates iff
        (initiation[a,f,si]
        or
        (not termination[a,f,si] and some si.prev and f->(si.prev) in a.initiates))

    // termination for a concrete action (Act)
    all a : Act, f : Fluent, si : FlActionIdx | f->si in a.terminates iff
        (termination[a,f,si]
        or
        (not initiation[a,f,si] and some si.prev and f->(si.prev) in a.terminates))
}

pred initiation[a : Act, f : Fluent, si : FlActionIdx] {
    let s = { s : f.flActions | s.baseName = a.baseName and s.flActIdx = si } |
        s.value = True
}

pred termination[a : Act, f : Fluent, si : FlActionIdx] {
    let s = { s : f.flActions | s.baseName = a.baseName and s.flActIdx = si } |
        s.value = False
}
...
pred isInitAct[a : Act, e : Env, f : Fluent] {
    ...
    let si = highestPriority[flActCategory] |
        (some si and f->si in a.initiates) and
        ((~(f.vars)).(flActCategory.flToActParamsMap).(a.params) in e.maps)
}
...
pred isTermAct[a : Act, e : Env, f : Fluent] {
    let flActCategory = { s : f.flActions | s.baseName = a.baseName } |
    let si = highestPriority[flActCategory] |
        (some si and f->si in a.terminates) and
        ((~(f.vars)).(flActCategory.flToActParamsMap).(a.params) in e.maps)
}
